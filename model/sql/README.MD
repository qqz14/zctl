...
zctl model 为go-zero下的工具模块中的组件之一，目前支持识别mysql ddl进行model层代码生成，通过命令行或者idea插件（即将支持）可以有选择地生成带redis cache或者不带redis cache的代码逻辑。
...
* 通过ddl生成
    ...
    ```shell script
    zctl model mysql ddl -src="./*.sql" -dir="./sql/model" -c
    ```
...
* 通过datasource生成
    ...
    ```shell script
    zctl model mysql datasource -url="user:password@tcp(127.0.0.1:3306)/database" -table="*"  -dir="./model"
    ```
...
```Plain Text
NAME:
   zctl model mysql - generate mysql model"

USAGE:
   zctl model mysql command [command options] [arguments...]
...
* 带缓存模式
  * ddl
	...
	```shell script
	zctl model mysql -src={patterns} -dir={dir} -cache
	```
	...
	```
	NAME:
       zctl model mysql ddl - generate mysql model from ddl
    
    USAGE:
       zctl model mysql ddl [command options] [arguments...]
    
    OPTIONS:
       --src value, -s value  the path or path globbing patterns of the ddl
       --dir value, -d value  the target dir
       --style value          the file naming format, see [https://github.com/zeromicro/go-zero/tree/master/tools/zctl/config/readme.md]
  ...
  * datasource
	...
	```shell script
	zctl model mysql datasource -url={datasource} -table={patterns}  -dir={dir} -cache=true
	```
	...
	```
	NAME:
       zctl model mysql datasource - generate model from datasource
    
    USAGE:
       zctl model mysql datasource [command options] [arguments...]
	...
	> NOTE: zctl model mysql ddl/datasource 均新增了一个`--style`参数，用于标记文件命名风格。
...
* 不带缓存模式
  ...
  * ddl
      ...
      ```shell script
        zctl model -src={patterns} -dir={dir}
      ```

  * datasource
  
      ```shell script
        zctl model mysql datasource -url={datasource} -table={patterns}  -dir={dir}
      ```

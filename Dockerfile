...
COPY . .
RUN go mod download
RUN go build -ldflags="-s -w" -o /app/zctl ./zctl.go
...
COPY --from=builder --chown=1000:1000 /go/bin/protoc-gen-go* /app/zctl /usr/local/bin/
...
LABEL org.opencontainers.image.title="zctl (cli)"

ENTRYPOINT ["/usr/local/bin/zctl"]

...
type LintResult struct {
	...
	// BuildFailed is true when the project failed to compile and no linters ran.
	// All lint-derived tabs will be empty; only "build-error" tab has content.
	BuildFailed bool
}

var lastLintResult *LintResult

// LastLintResult returns the LintResult from the most recent RunLint call.
func LastLintResult() *LintResult { return lastLintResult }
...
func RunLint(dir, outDir string) *Result {
	...
	// ── Detect typecheck-only failure (compilation error, no linters ran) ──
	// golangci-lint aborts all linters when the project fails to compile.
	// In that case the JSON contains only typecheck issues.
	// We surface this as a distinct "build failed" result so the user knows
	// that ALL other lint checks were skipped — not that the code is clean.
	if isTypecheckOnly(rawIssues) {
		var buildErrors []string
		for _, iss := range rawIssues {
			// typecheck Text contains multi-line output; split into individual lines
			for _, line := range strings.Split(iss.Text, "\n") {
		...
		r := Fail(
			fmt.Sprintf("build failed — %d package(s) did not compile; all linters skipped", len(rawIssues)),
			buildErrors,
		)
		tabs := map[string][]string{"build-error": buildErrors}
		lastLintResult = &LintResult{Result: *r, Tabs: tabs}
		lastLintResult.BuildFailed = true
		return r
	}
...
// isTypecheckOnly returns true when every issue comes from "typecheck",
// which means the project failed to compile and no real linters ran.
func isTypecheckOnly(issues []lintIssue) bool {
	if len(issues) == 0 {
		return false
	}
	for _, iss := range issues {
		if iss.FromLinter != "typecheck" {
			return false
		}
	}
	return true
...
// golangciYmlVersion is embedded in defaultGolangciYml as a sentinel comment.
// Bump this when the default config changes to force an update in existing projects.
const golangciYmlVersion = "zctl-perf-v4"

func ensureGolangciYml(dir string) {
	ymlPath := filepath.Join(dir, ".golangci.yml")
	if fileExists(ymlPath) {
		// Only overwrite if the file was generated by us AND the version is outdated.
		data, err := os.ReadFile(ymlPath)
		if err != nil {
			return
		}
		content := string(data)
		// If the file wasn't generated by zctl, leave it alone.
		if !strings.Contains(content, "auto-generated by zctl perf scan") {
			return
		}
		// If it already has the current version tag, no update needed.
		if strings.Contains(content, golangciYmlVersion) {
			return
		}
		// Outdated zctl-generated config → overwrite with latest.
	}
...
issues:
  exclude-dirs:
    ...
    - types
    - migrations
    - mock         # mock 文件只用于测试，编译问题不应中断全局 lint
  exclude-rules:
    - path: "_test\\.go"

...
This is the simplest example of generating an RPC service with zctl.
...
### Method 1: Quick Start with `zctl rpc new`
...
- The `--module` flag tells zctl the Go module name; `--go_opt=module=...` and `--go-grpc_opt=module=...` tell protoc to strip the module prefix from output paths.
- The `--zrpc_out` flag specifies where the zctl-generated service code goes.

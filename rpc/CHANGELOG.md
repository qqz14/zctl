...
When an imported proto has a **different** `go_package` from the main proto, zctl now automatically generates the correct Go import paths and qualified type references in server, logic, and client code.
...
Previously, zctl rejected any RPC request/response type containing a dot (e.g., `base.Req`), requiring all types to be defined in the same proto file. This restriction has been removed.

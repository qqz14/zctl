# zctl rpc — RPC Code Generation

English | [中文](README-cn.md)

zctl rpc is the RPC service code generation module of the `zctl` scaffold. It generates a complete zRPC service from `.proto` files. You only need to write the proto definition and business logic — all boilerplate code is generated automatically.
...
```bash
zctl rpc template -o=user.proto
```
...
```bash
zctl rpc protoc <proto_file> [flags]
```
...
zctl rpc protoc multi.proto \
  --go_out=output --go-grpc_out=output --zrpc_out=output \
  --go_opt=module=example.com/demo --go-grpc_opt=module=example.com/demo \
  --module=example.com/demo -I . -m

# Import external protos
zctl rpc protoc service.proto \
  --go_out=output --go-grpc_out=output --zrpc_out=output \
  --go_opt=module=example.com/demo --go-grpc_opt=module=example.com/demo \
...
| `--home` | | string | | zctl template directory |
...
```bash
zctl rpc new <service_name> [flags]
```
...
```bash
zctl rpc template -o=<output_file> [flags]
```
...
- **Transitive imports**: A imports B, B imports C — zctl resolves recursively
...
zctl automatically recognizes and handles Google protobuf well-known types:
...
These types can be used directly as RPC parameter types — zctl generates the correct imports automatically.

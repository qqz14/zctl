...
// RegisterGoctlHome register zctl home path.
...
// GetGoctlHome returns the path value of the zctl, the default path is ~/.zctl, if the path has
...
// GetDefaultGoctlHome returns the path value of the zctl home where Join $HOME with .zctl.
func GetDefaultGoctlHome() (string, error) {
	var zctlHomeDir = zctlDir
	home, err := os.UserHomeDir()
	if err == nil {
		zctlHomeDir = filepath.Join(home, zctlDir)
	}

	_ = MkdirIfNotExist(zctlHomeDir)
	return zctlHomeDir, nil
}

// GetGitHome returns the git home of zctl.
func GetGitHome() (string, error) {
	zctlH, err := GetGoctlHome()
	if err != nil {
		return "", err
	}

	return filepath.Join(zctlH, gitDir), nil
}

// GetAutoCompleteHome returns the auto_complete home of zctl.
func GetAutoCompleteHome() (string, error) {
	...
	return filepath.Join(zctlH, autoCompleteDir), nil
}

// GetCacheDir returns the cache dit of zctl.
func GetCacheDir() (string, error) {
	...
	return filepath.Join(zctlH, cacheDir), nil
}
